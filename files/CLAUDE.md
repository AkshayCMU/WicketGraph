...
Build three interconnected components:

**2a. `src/analyzer.py` — Milestone Stats Engine**
Query the silver layer to surface cricket-grounded insights. Each function is standalone, typed, and MCP-ready:

- How did a bowler set up a batter before dismissal?
- What patterns appeared in the last 6–12 balls faced by a batter?
- Which over or spell shifted match momentum?
- How did a batter score against pace vs spin, by phase?
- Career stats, phase splits, head-to-head matchups, milestone progress tracking.

Milestone examples the engine must be able to answer:
- "Kohli needs 23 more runs vs left-arm spin in overs 8–15 to reach 500 in that context."
- "Bumrah has taken 40% of his IPL wickets in the powerplay vs right-hand batters."
- "Dhoni averages 58 in death overs when chasing with <15 needed off the last over."

**2b. `src/mcp_server.py` — MCP Tool Server**
Register every analyzer function as an MCP tool using the MCP Python SDK. Must run as a stdio server compatible with Claude Desktop and other MCP clients. Tool signatures must mirror function signatures exactly so LLM clients can invoke them without custom glue code.

**2c. `src/commentator_prep.py` — Commentator Briefing UI**
Takes a match input (e.g., "MI vs CSK"), loads a hardcoded IPL 2025 squad JSON, runs each player through the analyzer functions, calls the Claude API to generate narrative commentator briefings per player, and renders everything in a Streamlit UI with:
- Per-player cards showing milestone alerts and key stats
- Phase split charts (powerplay / middle / death)
- Pace vs spin performance tables
- AI-generated talking points for broadcast use

The Streamlit UI is the only sanctioned frontend in Phase 2. Do not build other dashboards.
